...
public class BreadcrumbAdvice {
    ...
    public List<Breadcrumb> breadcrumbs(HttpServletRequest request) {
        ...
        crumbs.add(new Breadcrumb("Главная", "/"));
    ...
    private void generateClientBreadcrumbs(List<Breadcrumb> crumbs, Matcher matcher) {
        String clientId = matcher.group(1);
        String subPath = matcher.group(2);

        // Добавляем "Клиенты"
        crumbs.add(new Breadcrumb("Клиенты", "/clients"));
        ...
        try {
            ...
            if (clientNameOpt.isPresent()) {
                crumbs.add(new Breadcrumb(clientNameOpt.get(), "/clients/" + clientId));
            } else {
            ...
            if (subPath != null && !subPath.isEmpty()) {
                ...
                for (String part : subParts) {
                    if (!part.isEmpty()) {
                        ...
                        crumbs.add(new Breadcrumb(label, subPathBuilder.toString()));
                    }
                }
            }
    ...
    private void generateStandardBreadcrumbs(List<Breadcrumb> crumbs, String uri) {
        ...
        for (String part : parts) {
            ...
            crumbs.add(new Breadcrumb(label, url));
        }
    }

    private void generateTemplateBreadcrumbs(List<Breadcrumb> crumbs, Matcher matcher) {
        String clientId = matcher.group(1);
        ...
        try {
            ...
            crumbs.add(new Breadcrumb(typeLabel, "/clients/" + clientId + "/" + type));

            // Добавляем "Шаблоны"
            crumbs.add(new Breadcrumb("Шаблоны", "/clients/" + clientId + "/" + type + "/templates"));
                    ...
                    if (!part.isEmpty()) {
                        ...
                        if (part.matches("\\d+")) {
                            try {
                                ...
                                crumbs.add(new Breadcrumb(templateName, pathBuilder.toString()));
                            } catch (NumberFormatException e) {
                                crumbs.add(new Breadcrumb("Шаблон #" + part, pathBuilder.toString()));
                            }
                        } else {
                            // Это действие (create, edit, etc.)
                            String label = SEGMENT_NAMES.getOrDefault(part, part);
                            crumbs.add(new Breadcrumb(label, pathBuilder.toString()));
                        }
                    }
                }
            }

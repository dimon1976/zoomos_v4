...
description: "Use when working on HTTP redirect detection, redirect strategies (CurlStrategy, PlaywrightStrategy, HttpClientStrategy, JsoupStrategy, WebClientStrategy, FirefoxPlaywrightStrategy), ProxyPoolManager, AntiBlockConfig, UrlSecurityValidator, redirect-finder UI, async redirect processing. Use when: редиректы не работают для конкретного сайта, нужна новая стратегия, прокси ротация, SSRF защита.\n\nExamples:\n- \"CurlStrategy не работает для goldapple.ru\"\n- \"Добавь поддержку meta-refresh редиректов\"\n- \"Прокси-ротация не работает\"\n- \"Playwright стратегия зависает на cloudflare\"\n- \"Нужна новая стратегия для антибот-защиты\"\n- \"SSRF уязвимость — нужна валидация URL\""
model: haiku
memory: project

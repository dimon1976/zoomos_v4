...
- Кнопка **«Все»** — toggle: если все статусы выбраны → сбросить все; иначе → выбрать все статусы `[CRITICAL, WARNING, TREND, IN_PROGRESS, OK]`
- Кнопка «Все» подсвечивается активной когда `activeFilters` пуст **или** содержит все 5 статусов

**Копирование проблем (`copyProblems`):**
- Фильтрует города со статусом CRITICAL/WARNING, **исключая** уже отмеченные (checked) города через `isCityChecked(siteName, cityId)`
- Это позволяет не дублировать в буфере уже обработанные города

**Зависшие выкачки (STALLED / NOT_FOUND):**
- Сообщение об ошибке STALLED теперь содержит: процент завершения, время последнего обновления, inStock с отклонением от медианы baseline
- Если выкачка не найдена (NOT_FOUND) но есть зависшая запись (completionPercent >= 50, не завершена) — показывается детальное сообщение вместо стандартного
